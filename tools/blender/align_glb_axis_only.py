...
def parse_args():
    ...
    p = argparse.ArgumentParser(
        "GLB 仅绕 Z 轴旋转以实现 L/M/S → X/Y/Z 映射（不移位/不缩放）"
    )
    ...
    p.add_argument(
        "--overwrite", action="store_true", help="覆盖原 .glb；否则在文件名后加后缀"
    )
    p.add_argument(
        "--suffix", default="_axis", help="未覆盖时输出文件名后缀（默认 _axis）"
    )
    p.add_argument(
        "--prefer-deg",
        type=int,
...
def get_vertices_world(objs):
    ...
    return (
        np.asarray(vs, dtype=np.float64) if vs else np.empty((0, 3), dtype=np.float64)
    )
...
def choose_rotation_by_axis_map_zonly(P_world, center_np, rank_map, prefer_deg=0):
    ...
    if prefer_deg not in (0, 90, 180, 270):
        prefer_deg = 0
    ...
    for deg in (0, 90, 180, 270):
        ...
        yaw_cost = (
            ...
            if (deg - prefer_deg) % 360 in (90, 270)
            else 2
        )
...
def apply_rotation_around(objs, R4, pivot):
    ...
    if copied:
        log(
            f"  已复制 {copied} 个共享 Mesh 数据块，避免 multi-user transform_apply 失败"
        )
    for ob in mesh_objs:
        ob.select_set(True)
    if not mesh_objs:
    ...
    for i, glb in enumerate(files, 1):
        try:
            ...
            R = choose_rotation_by_axis_map_zonly(
                P,
                np.array([pivot.x, pivot.y, pivot.z]),
                rank_map,
                prefer_deg=a.prefer_deg,
            )
