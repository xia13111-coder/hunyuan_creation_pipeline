...
def parse_args():
    ...
    import argparse
    p = argparse.ArgumentParser("将 GLB 缩放到目标 X/Y/Z 长度，并把包围盒中心移到原点后导出")
    ...
    p.add_argument("--unit", choices=["m","cm","mm"], default="m", help="以上长度的单位（默认 m）")
    p.add_argument("--overwrite", action="store_true", help="覆盖原 .glb；否则加后缀另存")
    p.add_argument("--suffix", default="_scaled", help="未覆盖时输出后缀（默认 _scaled）")
...
def apply_matrix_to_group(objs, M):
    ...
    if any(ob.select_get() for ob in objs if ob.type=="MESH"):
        bpy.context.view_layer.objects.active = [ob for ob in objs if ob.type=="MESH"][0]
...
def main():
    ...
    for i, glb in enumerate(files, 1):
        try:
            ...
            if np.any(ext <= 1e-9):
                log(f"  ! 包围盒某些轴长度≈0，无法缩放到目标值：{ext}"); continue

            scale = target / ext
            pivot = (mn + mx) * 0.5  # 包围盒中心（世界坐标）

            # 组合矩阵：先绕包围盒中心缩放，再把中心移到世界原点
            S4 = Matrix(((scale[0],0,0,0),
                         (0,scale[1],0,0),
                         (0,0,scale[2],0),
                         (0,0,0,1)))
