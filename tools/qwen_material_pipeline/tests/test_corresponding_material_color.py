    ...
    choices = _seal(
        {
            ...
            "assignment_unit": "part_id",
            "selections": [
                {"part_id": "P1", "material_id": EXACT, "match_type": "EXACT_LIBRARY_MATCH"},
                {"part_id": "P2", "material_id": PAINT, "match_type": "CORRESPONDING_MATERIAL"},
                {"part_id": "P3", "material_id": METAL, "match_type": "CORRESPONDING_MATERIAL"},
                {"part_id": "P4", "material_id": METAL, "match_type": "CORRESPONDING_MATERIAL"},
            ],
...
def test_group_uses_one_deterministic_photo_medoid() -> None:
    ...
    scope = next(row for row in audit["scopes"] if row["scope_id"] == "COMPONENT:C_GREEN")
...
def test_rejects_existing_parameters_or_material_drift() -> None:
    ...
    with pytest.raises(CorrespondingMaterialColorError, match="already contains parameters"):
        build_corresponding_material_color_plan(
            source_plan=source,
            qwen_choices=choices,
            part_id_evidence=evidence,
...
def test_rejects_exact_and_corresponding_members_mixed_in_one_component() -> None:
    ...
    choices = _seal({key: value for key, value in choices.items() if key != "integrity"})
    with pytest.raises(CorrespondingMaterialColorError, match="mixes library match types"):
        build_corresponding_material_color_plan(
            source_plan=source,
            qwen_choices=choices,
            part_id_evidence=evidence,
...
def test_rejects_component_member_outside_sealed_plans() -> None:
    ...
    choices = _seal({key: value for key, value in choices.items() if key != "integrity"})
    with pytest.raises(CorrespondingMaterialColorError, match="outside the sealed plans"):
        build_corresponding_material_color_plan(
            source_plan=source,
            qwen_choices=choices,
            part_id_evidence=evidence,
...
def test_rejects_unsealed_or_unsupported_color_evidence() -> None:
    ...
    evidence = _seal({key: value for key, value in evidence.items() if key != "integrity"})
