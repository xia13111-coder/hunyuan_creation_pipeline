...
class _TwoStageRunner:
    model_identity = {"backend": "fake", "fingerprint": "two-stage-unit-test"}
    ...
    def generate_with_metadata(self, payload: object) -> _Generation:
        ...
        if "First predict physical material semantics" in prompt:
            return _Generation(
                json.dumps(
                    {
...
class PartIdQwenTests(unittest.TestCase):
    ...
    def test_material_prediction_hard_filters_before_visual_ranking(self) -> None:
        paint = "mdl:Miscellaneous/Paint_Matte.mdl#Paint_Matte"
        water = "mdl:Natural/Water.mdl#Water"
        decision = _validate_physical_batch(
            {
                "schema_version": PHYSICAL_BATCH_SCHEMA_VERSION,
                "classifications": [
                    {
            ...
            expected=[{"part_id": "P0001"}],
        )[0]
        compatible, audit = _physical_filter_ranking(
            part_id="P0001",
            ranking=[
                {"rank": 1, "material_id": water},
                {"rank": 2, "material_id": paint},
            ],
            catalog_by_id={
                water: {
                    "material_id": water,
                    "surface_semantics": self._surface(
                        substrates=["liquid"],
                        treatment="bare",
                        optical="transparent",
                    ),
        ...
        self.assertEqual([row["material_id"] for row in compatible], [paint])
        self.assertEqual(audit["policy"], "predict_material_then_filter_mdl/v1")
        self.assertFalse(audit["visual_cross_family_fallback_allowed"])

    def test_material_prediction_expands_all_compatible_catalog_candidates(self) -> None:
        plastic_a = "mdl:Plastics/Plastic_ABS.mdl#Plastic_ABS"
        plastic_b = "mdl:Plastics/Plastic.mdl#Plastic"
        decision = _validate_physical_batch(
            {
                "schema_version": PHYSICAL_BATCH_SCHEMA_VERSION,
                "classifications": [
        ...
        compatible, audit = _physical_filter_ranking(
            part_id="P0001",
            ranking=[{"rank": 1, "material_id": plastic_a}],
            catalog_by_id={
                material_id: {
                    "material_id": material_id,
                    "surface_semantics": self._surface(
                        substrates=["polymer"], treatment="bare"
                    ),
                }
                for material_id in (plastic_a, plastic_b)
            },
            decision=decision,
        )
        self.assertEqual(
            [row["material_id"] for row in compatible],
            [plastic_a, plastic_b],
        )
        self.assertEqual(audit["compatible_retrieved_candidate_count"], 1)
        self.assertEqual(audit["compatible_catalog_expansion_count"], 1)
        self.assertTrue(audit["full_catalog_expansion_used"])

    def test_material_prediction_rejects_low_confidence_forced_category(self) -> None:
        with self.assertRaisesRegex(PartIdQwenError, "below the confidence floor"):
            _validate_physical_batch(
                {
                    "schema_version": PHYSICAL_BATCH_SCHEMA_VERSION,
                    "classifications": [
    ...
    def test_strict_identity_prompt_does_not_optimize_across_categories(self) -> None:
        ...
        prompt = payload["messages"][1]["content"][-1]["text"]
        self.assertIn("Material category is immutable", prompt)
        self.assertIn("not by their default swatch color", prompt)
        self.assertNotIn("Exact engineering/semantic category may differ", prompt)

    def test_full_two_stage_run_predicts_material_before_mdl_choice(self) -> None:
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            crop = root / "crop.png"
            Image.new("RGB", (16, 16), (30, 160, 60)).save(crop)
            plastic_a = "mdl:Plastics/Plastic_ABS.mdl#Plastic_ABS"
            plastic_b = "mdl:Plastics/Plastic.mdl#Plastic"
            water = "mdl:Natural/Water.mdl#Water"
                    ...
                    "groups": [
                        {
                            ...
                            "fused_ranking": [
                                {"rank": 1, "material_id": water},
                                {"rank": 2, "material_id": plastic_a},
                                {"rank": 3, "material_id": plastic_b},
                            ],
                        }
                    ]
                },
        ...
        self.assertIn("First predict physical material semantics", runner.calls[0])
        self.assertIn("Material category is immutable", runner.calls[1])
        self.assertIn(result["choices"]["P0001"], {plastic_a, plastic_b})
        self.assertNotEqual(result["choices"]["P0001"], water)
        self.assertEqual(result["summary"]["material_prediction_count"], 1)
