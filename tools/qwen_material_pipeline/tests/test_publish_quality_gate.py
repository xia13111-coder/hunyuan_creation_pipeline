...
def _low_confidence_part_id_lineage() -> tuple[dict, dict, dict, dict, dict]:
    ...
    rejected_material = "mdl:Base/Glass/Green_Glass.mdl#Green_Glass"
    base_assignments = [
        {
            "part_id": "P0001",
...
def test_observed_low_confidence_lineage_rejects_forgery(
    ...
    if mutation == "material":
        ...
        assignment["provenance"][
            "selected_reference_view_id_for_rejected_qwen"
        ] = "front"
        audit["parts"][0]["evidence_view_ids"] = ["front"]
    elif mutation == "wrong_summary":
        audit["summary"]["observed_low_confidence_baseline_retained_count"] = 0
        ...
        digest = _sha256(unsigned_evidence)
        evidence["integrity"] = {"document_sha256": digest}
        final_plan["provenance"]["part_id_evidence_sha256"] = digest
        audit["part_id_evidence_sha256"] = digest
