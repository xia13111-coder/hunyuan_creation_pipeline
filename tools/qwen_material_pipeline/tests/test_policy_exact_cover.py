...
def _sealed_part_id_visibility(
    registry: dict,
    *,
    unobserved_part_ids: set[str],
) -> dict:
    part_ids = [str(part["part_id"]) for part in registry["parts"]]
    parts = [
        {
            "part_id": part_id,
            "status": (
                "unobserved" if part_id in unobserved_part_ids else "observed"
            ),
            "observations": (
                []
        ...
        "inputs": [
            {
                ...
                "path": None,
                "document_sha256": policy_module._canonical_sha256(registry),
            }
        ],
        "parts": parts,
        "summary": {
            "registry_part_count": len(parts),
            "observed_part_count": len(parts) - len(unobserved_part_ids),
            "unobserved_part_count": len(unobserved_part_ids),
        },
    }
    return {
        **unsigned,
        "integrity": {"document_sha256": policy_module._canonical_sha256(unsigned)},
    }
...
def test_final_part_id_visibility_neutralizes_unobserved_p0181_group_attack() -> None:
    ...
    # source-appearance signature and repeated geometry that made G05 look
    # plausible, but the final one-to-one Part-ID projection sees no pixels.
    registry["parts"][3]["part_id"] = "P0181"
    gate["decisions"][3]["part_id"] = "P0181"
    evidence = _sealed_part_id_visibility(
        registry,
        unobserved_part_ids={"P0181"},
    )
    blue_polycarbonate = (
        "mdl:vMaterials_2/Plastic/Polycarbonate_Opaque.mdl#Polycarbonate_Blue"
    )
    ...
    unconverged_plan, _unconverged_report = build_policy_exact_cover(
        registry=registry,
        staged_result=staged,
        confidence_gate=gate,
        whitelist=whitelist,
    ...
    unconverged = {
        row["part_id"]: row for row in unconverged_plan["assignments"]
    }["P0181"]
    assert unconverged["material_id"] == blue_polycarbonate
    assert unconverged["provenance"]["canonical_group_id"] == "G01"
    ...
    p0181 = {row["part_id"]: row for row in plan["assignments"]}["P0181"]
    assert p0181["material_id"] == STAINLESS_STEEL_MATTE
    assert p0181["status"] == FALLBACK_STATUS
    assert p0181["confidence"] == 0.0
    assert p0181["evidence_views"] == []
    assert p0181["provenance"]["tier"] == (
        "source_preserve_unavailable_neutral_fallback"
    )
    assert not {
        "canonical_group_id",
        ...
        "group_id",
    }.intersection(p0181["provenance"])
    assert "P0181" not in report["corroborated_source_visual"][
        "applied_part_ids"
    ]
    assert report["summary"]["corroborated_source_visual_nvidia_mdl_count"] == 3
    assert report["summary"]["part_id_evidence_unobserved_count"] == 1
    assert report["part_id_evidence_convergence"][
        "unobserved_assignment_policy"
    ] == "independent_policy_only_no_palette_group_or_identity_propagation"
    assert plan["provenance"]["part_id_evidence_sha256"] == (
        policy_module._canonical_sha256(evidence)
    )


def test_part_id_visibility_convergence_rejects_a_resealed_status_mismatch() -> None:
    registry, staged, gate, whitelist, palette = _source_accent_bundle()
    evidence = _sealed_part_id_visibility(
        registry,
        unobserved_part_ids={"P0004"},
    )
    evidence["parts"][3]["status"] = "observed"

    with pytest.raises(PolicyExactCoverError, match="integrity seal"):
        build_policy_exact_cover(
            registry=registry,
            staged_result=staged,
            confidence_gate=gate,
