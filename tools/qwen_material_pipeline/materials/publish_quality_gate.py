        ...
        if row_status == "independently_selected":
            ...
            if (
                ...
                or any(
                    ...
                    for key in ("semantic", "confidence", "evidence_views")
                )
                or row.get("material_id") != final_assignment.get("material_id")
                ...
                or row.get("mdl_color_parameterization") != color_parameterization
                or any(
                    key in provenance
                    for key in (
    ...
    *,
    ...
    coverage_denominator = (
        (
            ...
            + len(part_id_lineage["observed_low_confidence_retained_part_ids"])
        )
        if part_id_lineage is not None
    ...
    scoped_policy_fallback_count = (
        sum(
            ...
            for part_id in (
                part_id_lineage["independently_selected_part_ids"]
                + part_id_lineage["observed_low_confidence_retained_part_ids"]
            )
        )
        if part_id_lineage is not None
    ...
    scoped_neutral_fallback_count = (
        int(part_id_lineage["observed_low_confidence_retained_neutral_count"])
        if part_id_lineage is not None
