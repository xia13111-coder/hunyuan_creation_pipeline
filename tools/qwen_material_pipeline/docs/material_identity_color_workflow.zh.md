...
逐零件边界同时保留目标在 CAD 整机图中相对其他零件的位置。模型图目标形状通过封存的整机
投影生成二维参考图候选，自动尺度的相似变换只允许在原 CAD 位置附近搜索；SAM3、EntitySeg、
照片边缘和模型形状共同评分，候选不得侵入 CAD 邻件内部。只有所有安全量不退化时才替换原
基线；极小掩码使用全分辨率注册回退，任何新候选被拒绝时仍输出已审计基线而不终止流程。
同一 Part-ID 的多个可见连通块必须作为并集生成最终候选，逐块变体只能参与评分，不能造成
同一零件的局部漏分。
