...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, hidden, xpos=None, return_inter=False):
        hidden = self.projects(hidden)
        intermediates = [hidden]
...
class LinearPts3d (nn.Module):
    ...
    def forward(self, decout, img_shape):
        ...
        feat = feat.transpose(-1, -2).view(B, -1, H//self.patch_size, W//self.patch_size) # B,3*patch_size^2,patch_h,patch_w
        feat = F.pixel_shuffle(feat, self.patch_size)  # B,3,H,W

        # permute + norm depth
        return feat.permute(0, 2, 3, 1)

# Used for refine details of intrinsic prediction
class RefinementHead(nn.Module):
    def __init__(self, in_channels=3, hidden_dim=32, num_blocks=3):
        super().__init__()
        layers = [nn.Conv2d(in_channels, hidden_dim, kernel_size=3, padding=1), nn.ReLU(inplace=True)]
        for _ in range(num_blocks - 1):
            layers += [
                nn.Conv2d(hidden_dim, hidden_dim, kernel_size=3, padding=1),
                nn.ReLU(inplace=True)
            ]
        layers.append(nn.Conv2d(hidden_dim, in_channels, kernel_size=3, padding=1))
