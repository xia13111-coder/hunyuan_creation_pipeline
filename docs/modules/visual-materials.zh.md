...
当装配投影本身含有错误的内部切口时，EntitySeg 即使因外部越界而不能作为整块掩码，仍可
在高可见召回、位置一致的条件下提供局部修复证据。修复范围只限于 isolated mesh 内部的
封闭孔洞，或按当前零件投影尺寸自动计算后可闭合的窄缺口；EntitySeg 对整件外轮廓没有权限，
与 mesh 外缘相连的宽遮挡仍是硬背景。修复只有在照片边缘、完整 mesh 和当前视图可见性联合
目标上确实改善时才会进入最终掩码，因此不依赖零件编号或特定资产阈值。
...
| --- | --- |
| SAM3 | 提供整机前景和逐零件迭代初值。 |
| EntitySeg / CropFormer | 提供类无关迭代初值，并在安全门内修复有照片支持的 CAD 内部缺口。 |
