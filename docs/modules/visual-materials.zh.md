...
目标 Part-ID 还会保留在 CAD 整机渲染图中的坐标和邻件关系。流程先通过封存的整机投影把
模型图零件形状注册成参考图候选，再只在按目标包围盒自动界定的相似变换范围内比较照片边缘、
SAM3/EntitySeg 支持和模型形状。候选不得侵入按零件尺度腐蚀后的 CAD 邻件内部；只有照片边缘
和模型形状不退化、候选支持不低于原 CAD 位置时才可替换基线。极小零件若在快速注册降采样中
消失，会自动改用全分辨率质心、主轴和面积注册；候选失败只回退基线，不中断整批运行。
同一 Part-ID 被遮挡成多个可见连通块时，逐块形状只用于评分，最终候选必须保留与当前观测
对应的全部 CAD 连通块并集，不能因某一块得分更高而漏掉同一零件的其他区域。
...
| 迭代边界优化器 | 联合模型图零件形状、整机邻件相对位置、装配可见性、前序掩码和照片边缘；只变换二维候选掩码，绝不单独移动 mesh。 |
...
并用实际 CAD 重渲染逐组件、逐成员检查局部外观，避免小零件被整图平均分掩盖。
