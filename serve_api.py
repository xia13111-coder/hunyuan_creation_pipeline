...
def refresh_tencent_key_status() -> dict[str, Any]:
    checked_at = now_iso()
    try:
        identity = hunyuan_to3d_batch.validate_credentials(
            tencent_key_region(),
    ...
    except hunyuan_to3d_batch.TencentCloudSDKException as exc:
        status = {
            "valid": False,
            "checked_at": checked_at,
            "code": hunyuan_to3d_batch.get_sdk_error_code(exc) or "TencentCloudSDKException",
            "message": hunyuan_to3d_batch.get_sdk_error_message(exc),
            "account_id": None,
            "principal_id": None,
            "arn": None,
        }
...
class GenerateModelRequest(BaseModel):
    ...
    face_count: int | None = Field(default=None, description="映射到 hunyuan_to3d_batch.py 的 --face-count")
    download_preview: bool = Field(default=False, description="下载腾讯云返回的 PreviewImageUrl 预览图")
...
class ProcessModelRequest(BaseModel):
    ...
    orientation: str = Field(default="X=L,Y=M,Z=S", description="模型朝向，直接映射到 --axis-map")
    set_mass: float | None = Field(default=None, description="模型质量；为空时自动按体积和材质密度计算")
    material: str = Field(default="plastic", description="材质标签，对应 materials.json")
    approx: str = Field(default="convexDecomposition", description="碰撞体类型，对应 --approx")
...
class GenerateAndProcessModelRequest(BaseModel):
    ...
    refine_mesh: bool = Field(default=True, description="混元生成 GLB 后先执行 refine mesh，再进入 Blender/Isaac 后处理")
    refine_output_dir: str | None = Field(default=None, description="refine mesh 输出目录；为空时自动使用 output_dir + '_refined_mesh'")
    refine_config_path: str | None = Field(default=None, description="refine mesh 配置文件；为空时使用 configs/hunyuan_reduce_local_postprocess.yaml")
    refine_temp_upload: str | None = Field(default=None, description="临时公网上传服务；默认取 REFINE_MESH_TEMP_UPLOAD，未设置时为 uguu")
    refine_fail_on_qc_error: bool = Field(default=False, description="QC 状态为 fail 时是否让任务失败")
    len_x: float
    len_y: float
    len_z: float
...
def health() -> dict[str, Any]:
    return {
        ...
        "supported_materials": pipeline_runner.available_materials(),
        "supported_approx_types": pipeline_runner.available_approx_types(),
...
def generate_model(request: GenerateModelRequest) -> dict[str, Any]:
    ...
    if not (payload.get("input_dir") or payload.get("prompt") or payload.get("image_url")):
        raise HTTPException(status_code=400, detail="input_dir、prompt、image_url 至少提供一个")
    require_valid_tencent_key()
    return submit_job(
        "generate_model",
        payload,
        lambda log_cb: pipeline_runner.run_generate_model_job(log_cb=log_cb, **payload),
    )
...
def process_model(request: ProcessModelRequest) -> dict[str, Any]:
    ...
    return submit_job(
        "process_model",
        payload,
        lambda log_cb: pipeline_runner.run_process_model_job(log_cb=log_cb, **payload),
    )


@app.post("/jobs/generate-and-process-model")
def generate_and_process_model(request: GenerateAndProcessModelRequest) -> dict[str, Any]:
    payload = model_dump(request)
    if not (payload.get("input_dir") or payload.get("prompt") or payload.get("image_url")):
        raise HTTPException(status_code=400, detail="input_dir、prompt、image_url 至少提供一个")
    require_valid_tencent_key()
    return submit_job(
        ...
        payload,
        lambda log_cb: pipeline_runner.run_generate_and_process_model_job(log_cb=log_cb, **payload),
    )
